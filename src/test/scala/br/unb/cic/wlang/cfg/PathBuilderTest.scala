...
class PathBuilderTest extends AnyFunSuite {
  ...
  test("Test for complete path parsing the fibonacci program") {
    ...
    val intercfgFlow = CFGBuilder.interFlow(wp)
    ...
    val expected1 = List(17, 14, 7, 9, 14, 7, 8, 15, 10, 12, 14, 7, 8, 15, 13, 15, 18)
    val expected2 = List(17, 14, 7, 9, 14, 7, 8, 15, 18)
    val expected3 = List(17, 14, 7, 8, 15, 18)
    assert(completePath(expected1, cfgFlow, intercfgFlow))
    assert(!completePath(expected2, cfgFlow, intercfgFlow))
    assert(completePath(expected3, cfgFlow, intercfgFlow))
  }
}
